...
use crate::{from_grpc_response, from_grpc_response_v2, Client, GreptimeResp, Result};
...
impl Database {
    ...
    pub async fn handle_v2(&self, request: Request) -> Result<GreptimeResp> {
        let mut client = make_database_client(&self.client)?.inner;
        let request = self.to_rpc_request(request);
        let response = client.handle(request).await?.into_inner();
        from_grpc_response_v2(response)
    }
    ...
    pub async fn handle_with_retry_v2(
        &self,
        request: Request,
        max_retries: u32,
    ) -> Result<GreptimeResp> {
        ...
        loop {
            ...
            match (raw_response, retries < max_retries) {
                (Ok(resp), _) => return from_grpc_response_v2(resp.into_inner()),
                (Err(err), true) => {
