...
fn build_plan_helper(
    ...
    let mut builder = LogicalPlanBuilder::scan(
        ...
        scan_request.projection_input.projection.clone(),
    )?;
...
mod tests {
    ...
    fn test_manifest_build_plan() {
        let projection_input = ProjectionInput::new().with_projection(Some(vec![0, 1, 2]));
    ...
    fn test_storage_build_plan() {
        let projection_input = ProjectionInput::new().with_projection(Some(vec![0, 2]));
        let request = ScanRequest {
            projection_input,
