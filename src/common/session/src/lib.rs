...
pub enum ReadPreference {
    ...
    Follower,
    // Reads all operations from the region follower, but prefers to read from the leader if the follower is not available.
    #[strum(serialize = "follower_preferred", to_string = "FOLLOWER_PREFERRED")]
    FollowerPreferred,
}
