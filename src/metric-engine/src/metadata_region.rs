...
impl MetadataRegion {
    fn build_prefix_read_request(prefix: &str, key_only: bool) -> ScanRequest {
        ...
        let projection_input = ProjectionInput::new().with_projection(Some(projection));
        ScanRequest {
            projection_input,
...
impl MetadataRegion {
    ...
    pub async fn get(&self, region_id: RegionId, key: &str) -> Result<Option<String>> {
        ...
        let projection_input =
            ProjectionInput::new().with_projection(Some(vec![METADATA_SCHEMA_VALUE_COLUMN_INDEX]));
