...
mod tests {
    ...
    fn test_projection_indices() {
        ...
        // Only read tag1
        let read_format =
            PrimaryKeyReadFormat::new(metadata.clone(), &ReadColumns::from_column_ids([3]));
        assert_eq!(
            &[2, 3, 4, 5],
        ...
        // Only read field1
        let read_format =
            PrimaryKeyReadFormat::new(metadata.clone(), &ReadColumns::from_column_ids([4]));
        assert_eq!(
            &[0, 2, 3, 4, 5],
            read_format.parquet_read_columns().root_indices()
        );
        // Only read ts
        let read_format =
            PrimaryKeyReadFormat::new(metadata.clone(), &ReadColumns::from_column_ids([5]));
        assert_eq!(
            &[2, 3, 4, 5],
            read_format.parquet_read_columns().root_indices()
        );
        // Read field0, tag0, ts
        let read_format =
            PrimaryKeyReadFormat::new(metadata, &ReadColumns::from_column_ids([2, 1, 5]));
        assert_eq!(
            &[1, 2, 3, 4, 5],
            read_format.parquet_read_columns().root_indices()
        );
